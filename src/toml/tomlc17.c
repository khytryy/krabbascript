...
static void
scan_init(scanner_t* sp, const char* src, int len, char* errbuf, int errbufsz);
...
static toml_datum_t*
tab_emplace(toml_datum_t* tab, span_t key, const char** reason)
{
    ...
    while (!feof(fp))
    {
        ...
        if (top == max)
        {
            ...
            if (tmpmax < 0)
            {
                ...
                {
                    snprintf(result.errmsg,
                             sizeof(result.errmsg),
...
toml_result_t toml_parse(const char* src, int len)
{
    ...
    if (toml_option.check_utf8)
    {
        ...
        for (int i = 0; i < len;)
        {
            ...
            if (0xD800 <= ch && ch <= 0xDFFF)
            {
                ...
                snprintf(result.errmsg,
                         sizeof(result.errmsg),
                         "invalid UTF8 char \\u%04x on line %d",
                         ch,
                         line);
                goto bail;
            }
    ...
    // Alloc memory pool
    pp->pool = pool_create(len +
                           10); // add some extra bytes for NUL term and safety
    if (!pp->pool)
    {
        snprintf(result.errmsg, sizeof(result.errmsg), "out of memory");
        goto bail;
    ...
    for (int i = 0; i < keypart->nspan; i++)
    {
        ...
        if (value->type == TOML_ARRAY)
        {
            ...
            if (value->type != TOML_TABLE)
            {
                RETERROR(pp->ebuf,
                         lineno,
                         "array %s must be array of tables",
                         keypart->span[i].ptr);
                return NULL;
            }
            tab = value; // descend
...
static int
parse_inline_array(parser_t* pp, token_t tok, toml_datum_t* ret_datum)
{
    ...
    *ret_datum     = mkdatum(TOML_ARRAY);
    ...
    for (;;)
    {
        ...
        if (tok.toktyp == TOK_COMMA)
        {
            ...
            return RETERROR(pp->ebuf,
                            tok.lineno,
                            "syntax error while parsing array: unexpected "
                            "comma");
        }
...
static int
parse_inline_table(parser_t* pp, token_t tok, toml_datum_t* ret_datum)
{
        ...
        switch (p[1])
        {
            ...
            case 'x':
            {
                ...
                if (n < 0)
                {
                    return RETERROR(pp->ebuf,
                                    tok.lineno,
                                    "error converting UCS %s to UTF8",
                                    buf);
                }
                dst += n;
                p += 4;
    ...
    while (1)
    {
        if (S_MATCH3('"'))
        {
            if (S_MATCH4('"'))
            {
                ...
                if (S_MATCH6('"'))
                {
                    return RETERROR(sp->ebuf,
                                    sp->lineno,
                                    "detected sequences of 3 or more double "
                                    "quotes");
                }
                else
                {
...
static int
read_time(const char* p, int* hour, int* minute, int* second, int* usec)
{
    const char* pp = p;
    int         n;
